  ...
  const fetchCourseMeets = async () => {
    try {
      ...
      if (result.success) {
        ...
        console.warn(' Using fallback mode - backend not available');
        setMeets([]);
      }
    } catch (error) {
...
// Add keyframes for spinner animation - inject into a new style element to avoid CORS issues
if (typeof document !== 'undefined' && !document.getElementById('course-meets-spinner-styles')) {
  const styleElement = document.createElement('style');
  styleElement.id = 'course-meets-spinner-styles';
