...
/**
 * SECURITY FIX 3.29.1: Load external scripts safely without DOM manipulation
 * Loads Cashfree payment SDK via script tag in document.head
 * This replaces document.body.appendChild() pattern
 */
const loadCashfreeScript = (paymentResult, meetId) => {
  // Check if script already loaded
  if (window.Cashfree) {
    initializeCashfree(paymentResult, meetId);
    return;
  }

  // Create script element in a safe way
  ...
  // Append to head (not body) - safer location
  document.head.appendChild(script);
};

const initializeCashfree = (paymentResult, meetId) => {
  try {
    ...
    const checkoutOptions = {
      ...
      returnUrl: `${window.location.origin}/user/meets/${meetId}/payment-callback`,
    ...
    try {
      ...
      if (meet.is_paid_meet && meet.payment_status !== 'completed') {
        ...
        try {
          ...
          const { getStorageString, getStorageEmail, getStoragePhone } = require('../../utils/storageValidator');

          // Create Cashfree payment order
          const paymentResult = await createMeetPaymentOrder(id, userId, {
            customer_name: getStorageString('userName', 'Guest'),
            customer_email: getStorageEmail('userEmail', null),
            customer_phone: getStoragePhone('userPhone', null)
          });
          ...
          if (paymentResult.success) { // Store order_id in localStorage for callback verification
            ...
            // SECURITY FIX 3.29.1: Load external scripts via useEffect, not direct DOM manipulation
            // Removed: document.body.appendChild() for script loading
            // Use useEffect with script loader pattern instead
            loadCashfreeScript(paymentResult, id);
          }
        } catch (paymentError) {
