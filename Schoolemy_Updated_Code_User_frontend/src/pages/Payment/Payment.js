...
import { useAuth } from "../../Context/AuthContext"; // Bug Fix 2.11.7: Auth check
import PaymentMethodSelector from "../../components/Payment/PaymentMethodSelector"; // Bug Fix 2.11.1
import EMISelector from "../../components/Payment/EMISelector"; // Bug Fix 2.11.6
import { rupeesToPaise, validateAndConvertAmount } from "../../utils/currencyConverter"; // Bug Fix 2.11.2
import usePaymentFlow from "../../hooks/usePaymentFlow"; // Bug Fix 2.11.4 & 2.11.5
import { validatePaymentAmount, validatePaymentForm, verifyPaymentData, logPaymentValidationError, mapUiPaymentMethodToApi } from "../../utils/paymentValidator"; // SECURITY FIX 3.38.1

const PaymentPage = () => {
  const { courseId } = useParams();
  const location = useLocation();
  const navigate = useNavigate();
  const { isLoggedIn, userData, isLoading: authLoading } = useAuth(); // Bug Fix 2.11.7: Auth check

  // Detect if this is a tutor course payment (App.js: user/payment/tutor-course/:courseId)
  ...
  // Bug Fix 2.11.4 & 2.11.5: Use payment flow hook instead of individual states
  ...
  // Bug Fix 2.11.7: Check authentication before allowing access
  useEffect(() => {
    if (!authLoading && !isLoggedIn) {
      // Save payment intent and redirect to login
      const paymentIntent = {
        courseId,
        isTutorCourse,
  ...
  }, [isLoggedIn, authLoading, courseId, isTutorCourse, location.pathname, navigate]);
    ...
    const fetchCourseData = async () => {
      try {
        ...
        try {
          ...
          console.warn(" Payment SDK loading warning (non-critical):", sdkError.message);
          // Don't fail page load if SDK fails initially - it will retry on payment
        }

        // First fetch course details from appropriate API
        console.log("Fetching course from:", isTutorCourse ? `/tutor-courses/${courseId}` : `/api/v1/courses/${courseId}`);
        let courseRes;
        if (isTutorCourse) {
          ...
          courseRes = await api.get(`/api/v1/courses/${courseId}`);
        }
        ...
        if (courseData.success) {
          ...
          if (course?.price && (!course.price.breakdown || !course.price.breakdown.courseValue)) {
            ...
            // Step 1: Course value after discount (net payable base before tax & fees)
            const courseValue = Math.round(mrp * (1 - discountPercent / 100));

            // Step 2: 18% GST on MRP (9% CGST + 9% SGST on list price)
            const cgst = Math.round(mrp * 0.09);
            const sgst = Math.round(mrp * 0.09);
            const gstAmount = cgst + sgst;

            // Step 3: Combined MRP + GST (before transaction fee)
            const mrpPlusGst = mrp + gstAmount;

            // Step 4: Transaction fee 2% on (MRP + GST)
            const transactionFee = Math.round(mrpPlusGst * 0.02);

            // Total = MRP + gstTotal + transactionFee
            course.price.finalPrice = mrp + gstAmount + transactionFee;
          ...
          if (course?.price?.finalPrice) {
            ...
            console.warn(" Course price not available for EMI calculation");
            setEmiDetails({
              eligible: false,
        ...
        if (isLoggedIn) {
          try {
            const paymentStatusEndpoint = isTutorCourse 
              ? `/api/v1/payments/user/payment/tutor-course/${courseId}`
              : `/api/v1/payments/user/payment/status/${courseId}`;
            console.log(
                ...
                if (paymentStatusData.data.type === "emi") {
                  ...
                  try {
                    const monthlyDueRes = await api.get(
                      `/api/v1/payments/user/emi/monthly-due/${courseId}`
                    );
                    console.log(
  ...
  }, [courseId, isLoggedIn, isTutorCourse]);
    ...
    return new Promise((resolve, reject) => {
      ...
      const attemptLoad = (urlIndex) => {
        ...
        timeoutId = setTimeout(() => {
          cleanup();
          console.warn(`Timeout loading from ${currentUrl}, trying next source...`);
  ...
  const handlePayment = async () => {
    // Bug Fix 2.11.4: Clear error using hook method
    paymentFlow.clearError();

    // Validation
    if (!paymentFlow.agreedToTerms)
      return paymentFlow.setError("Please agree to the terms and conditions.");

    // Tutor courses only support one-time payment (no EMI)
    if (isTutorCourse && paymentFlow.paymentType === "emi") {
      setModal({
        show: true,
    ...
    if (paymentFlow.paymentType === "emi") {
      ...
      if (!paymentFlow.emiDueDay) return paymentFlow.setError("Please select an EMI due date.");
      if (!emiDetails?.monthlyAmount || !emiDetails?.totalAmount)
        return paymentFlow.setError("EMI configuration is incomplete.");
    }

    // Bug Fix 2.11.4: Use hook method to start loading
    paymentFlow.startLoading();
    ...
    try {
      ...
      // Bug Fix 2.11.2: Ensure amount is converted to paise consistently
      const amountInRupees = isTutorCourse
        ? course?.price?.finalPrice || 0
        : paymentFlow.paymentType === "emi"
        ? emiDetails?.monthlyAmount || 0
        : course?.price?.finalPrice || 0;

      // SECURITY FIX 3.38.1: Validate amount matches course price
      const amountValidation = validatePaymentAmount(
        amountInRupees,
        { ...course, isTutorCourse },
        paymentType,
        paymentType === 'emi' ? emiDetails : null
      );

      if (!amountValidation.isValid) {
        logPaymentValidationError('payment-submission', amountValidation.errors, {
          courseId,
      ...
      // SECURITY FIX 3.38.1: Additional validation for currency converter
      const currencyValidation = validateAndConvertAmount(amountInRupees, {
        minAmount: isTutorCourse ? 500 : 100,
        maxAmount: isTutorCourse ? 50000 : 500000
      });

      if (!currencyValidation.isValid) {
        throw new Error(`Currency validation failed: ${currencyValidation.errors.join(', ')}`);
      }

      // Build payment data - use tutorCourseId for tutor courses, courseId for regular courses
      // Bug Fix 2.11.1: Use selected payment method from hook instead of hardcoded
      const apiPaymentMethod = mapUiPaymentMethodToApi(paymentFlow.paymentMethod);

      const paymentData = isTutorCourse
        ? {
            // Tutor course payment data
            tutorCourseId: courseId, // Use tutorCourseId field for tutor courses
            amount: amountInRupees, // Amount in rupees (backend will convert to paise)
            paymentMethod: apiPaymentMethod, // Backend enum: instrument (CARD/UPI/…), not gateway id "cashfree"
            paymentType: "full", // Tutor courses always use full payment
            mobile: mobileString,
          }
        : {
            // Regular course payment data
            courseId,
            paymentType: paymentFlow.paymentType,
            paymentMethod: apiPaymentMethod,
            emiDueDay: paymentFlow.paymentType === "emi" ? Number(paymentFlow.emiDueDay) : undefined,
            amount: amountInRupees, // Amount in rupees (backend will convert to paise)
            mobile: mobileString,
          };

      // SECURITY FIX 3.38.1: Verify payment data before submitting to gateway
      const paymentVerification = verifyPaymentData(paymentData, course);
      if (!paymentVerification.isValid) {
        logPaymentValidationError('payment-verification', paymentVerification.errors, {
          courseId,
          submittedAmount: paymentData.amount,
      ...
      const paymentEndpoint = "/api/v1/payments/user/payment/create";
      ...
      try {
        cashfree.checkout(checkoutOptions).then((result) => {
          if (result.error) {
            // Bug Fix 2.11.4: Handle error properly with hook method
            console.error(" Cashfree checkout error:", result.error);
            paymentFlow.handleError(result.error.message || "Payment failed");
            setModal({
              show: true,
              message: `Payment failed: ${result.error.message}. You can try again.`,
              isSuccess: false,
            });
            ...
            console.log(" Checkout closed or cancelled by user");
            paymentFlow.clearSuccess(); // Reset any previous success state
          }
        }).catch((checkoutError) => {
          console.error(" Checkout error:", checkoutError);
          // Bug Fix 2.11.4: Handle error with hook method
          paymentFlow.handleError("An error occurred during checkout. Please try again.");
          setModal({
            show: true,
            message: `Checkout error: ${checkoutError.message || "Unknown error"}. Retry available.`,
            isSuccess: false,
          });
        });
      ...
      // Bug Fix 2.11.4: Use hook method to handle error and allow retry
      paymentFlow.handleError(err.message);
      setPaymentProcessing(false);
      setModal({
        show: true,
        message: `Payment failed: ${err.message}. ${paymentFlow.canRetry() ? 'You can retry.' : paymentFlow.getRetryMessage()}`,
        isSuccess: false,
      });
    }
  };
  ...
  /** Same success payload as PaymentCallback — required for course content “payment success” UI */
  const navigateToCourseContentWithPaymentSuccess = () => {
    navigate(
      isTutorCourse
  ...
  const handlePaymentSuccess = async (orderId, paymentId) => {
    try {
      ...
      // Bug Fix 2.11.5: Clear stale payment data from localStorage after successful payment
      paymentFlow.clearPaymentData();
      ...
      const verifyEndpoint = "/api/v1/payments/user/payment/verify";
      ...
      if (verifyData.success) {
        ...
        // Bug Fix 2.11.4: Use hook to handle success properly
        paymentFlow.handleSuccess("Payment successful!");
        ...
        setTimeout(() => {
          navigateToCourseContentWithPaymentSuccess();
        }, 1500);
  ...
  const closeModal = () => {
    ...
    if (wasSuccess) navigateToCourseContentWithPaymentSuccess();
  };
                        ...
                        <div className="emi-status-section">
                          <span className={`status-badge ${emi.status}`}>
                            {emi.status === 'paid' ? ' Paid' :
                             ...
                             emi.status === 'overdue' ? ' Overdue' : emi.status}
                          </span>
                          {emi.dueDate && (
              ...
              {paymentType !== "emi" && (
                <div className="price-breakdown-section">
                  ...
                  <div className="summary-row" style={{ fontWeight: "600" }}>
                    ...
                    <span>
                      ₹
                      {(course.price.breakdown?.mrpPlusGst != null
                        ? course.price.breakdown.mrpPlusGst
                        : (course.price.amount || 0) +
                          (course.price.breakdown?.gst?.total || 0)
                      ).toLocaleString("en-IN")}
                    </span>
                  </div>
                  <div className="summary-row">
                    <span>Transaction Fee (2% of MRP + GST)</span>
                    <span>
                      ₹
