...
class EMIService {
  ...
  static async getEMIStatus(courseId) {
    try {
      const response = await api.get(`/api/v1/payments/user/emi/status/${courseId}`);
  ...
  static async getMonthlyDueAmount(courseId) {
    try {
      const response = await api.get(`/api/v1/payments/user/emi/monthly-due/${courseId}`);
      return response.data;
    } catch (error) {
  ...
  static async getUserEmiSummary() {
    try {
      const response = await api.get("/api/v1/payments/user/emi/summary");
      return response.data;
    } catch (error) {
  ...
  static async getEmiDueAmounts(courseId) {
    try {
      const response = await api.get(`/api/v1/payments/user/emi/due-amounts/${courseId}`);
      return response.data;
    } catch (error) {
  ...
  static async payMonthlyEmi(courseId, amount) {
    try {
      const response = await api.post("/api/v1/payments/user/emi/pay-monthly", {
        courseId,
        amount,
        paymentType: "monthly",
      });
  ...
  static async payOverdueEMIs(courseId, amount) {
    try {
      const response = await api.post("/api/v1/payments/user/emi/pay-overdue", {
        courseId,
        amount,
      });
      return response.data;
  ...
  static async getEMIDetails(courseId) {
    try {
      const response = await api.get(`/api/v1/payments/user/payment/emi-details/${courseId}`);
      return response.data;
    } catch (error) {
  ...
  static async verifyEmiPayment(paymentData) {
    try {
      const response = await api.post("/api/v1/payments/user/emi/verify-payment", paymentData);
      return response.data;
    } catch (error) {
