...
const normalizePhoneNumber = (mobile) => {
  // Validate that mobile is a string
  if (!mobile || typeof mobile !== "string") {
    logger.warn(`Invalid phone number: mobile must be a string, received ${typeof mobile}`);
    return null;
  }
  ...
  logger.warn(`Invalid phone number format: ${mobile}`);
  return null;
};
...
export const sendNotification = async (userId, type, data) => {
  ...
  data.isEmi ? data.totalAmount : data.amountPaid
              ...
              ${data.invoiceNumber ? `<li><strong>Invoice Number:</strong> <span class="highlight">${data.invoiceNumber}</span></li>` : ""}
            </ul>
            ${
  data.isEmi
    ? `
  ...
  data.noRefundPolicy
          ...
          <div class="footer">
            <p>Contact us at <a href="mailto:schoolemygkvk@gmail.com" style="color: #666666;">schoolemygkvk@gmail.com</a> </p>
          </div>
        </div>
      </body>
      </html>
  ...
  data.courseName
              ...
              <div class="urgent-card">
                <div style="text-align: center;">
                  ...
                  <div style="font-size: 14px; color: #c62828; margin-top: 8px; font-weight: 600;"> Payment Overdue</div>
                </div>
              </div>
              ...
              <div class="alert-box">
                <p> WARNING: Your course access will be locked if payment is not received by ${data.gracePeriodEnd.toDateString()}</p>
              </div>
  ...
  const smsTemplates = {
    ...
    lock: (user, data) =>
      `${data.courseName} access locked due to overdue EMIs. Pay all dues to unlock: ${data.paymentUrl}. Contact schoolemygkvk@gmail.com`,
    unlock: (user, data) =>
  ...
  if (user.email) {
    try {
      ...
      logger.debug(`Email sent: ${type} to ${user.email}`);
    } catch (error) {
  ...
  if (user.mobile) {
    ...
    if (normalizedMobile) {
      try {
        ...
        logger.debug(`SMS sent: ${type} to ${user.mobile}`);
      } catch (error) {
        logger.error(`Failed to send SMS to ${user.mobile}:`, error);
      }
      ...
      if (user.whatsappOptIn) {
        try {
          ...
          logger.error(`Failed to send WhatsApp to ${user.mobile}:`, error);
        }
      } else {
        logger.debug(
  ...
  logger.debug(`Notification sent: ${type} to user ${userId}`);
};
