...
const EMISchema = new mongoose.Schema({
  ...
  amount: { type: Number, required: true }, // Total monthly EMI amount (base + GST + txn fee)
  // Per-month GST & fee breakdown
  breakdown: {
    baseValue: { type: Number },    // Monthly course value (before GST/fees)
    cgst: { type: Number },         // CGST (9%) per month
    sgst: { type: Number },         // SGST (9%) per month
    gstTotal: { type: Number },     // Total GST (18%) per month
    transactionFee: { type: Number }, // Transaction fee (2%) per month
  },
  status: {
    type: String,
...
const EMIPlanSchema = new mongoose.Schema(
  {
    ...
    coursePrice: { type: Number, required: true }, // Total course price (finalPrice including GST)
    courseduration: { type: String, required: true }, // Course duration

    // GST Breakdown (copied from course at plan creation for transparency)
    priceBreakdown: {
      courseValue: { type: Number }, // Price after discount (before GST/fees)
      gst: {
        cgst: { type: Number }, // CGST (9%)
        sgst: { type: Number }, // SGST (9%)
    ...
    monthlyAmount: { type: Number, required: true }, // Monthly EMI installment amount
    ...
    processingFee: { type: Number, default: 0 }, // Processing fee if any
    status: {
      type: String,
