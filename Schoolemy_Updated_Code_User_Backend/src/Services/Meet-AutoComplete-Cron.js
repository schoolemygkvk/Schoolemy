...
export const autoCompleteMeets = async () => {
  try {
    ...
    logger.debug(`[Meet Auto-Complete] Found ${meetsToComplete.length} meets to auto-complete`);
    ...
    for (const meet of meetsToComplete) {
      ...
      if (now >= graceEnd) {
        ...
        logger.debug(`[Meet Auto-Complete] Completed meet: ${meet.meet_id} - ${meet.title}`);
        logger.debug(`  - Auto-completed: ${joinedParticipants.length} participants`);
        logger.debug(`  - Marked absent: ${notJoinedParticipants.length} participants`);
      }
    }
    ...
    logger.error("[Meet Auto-Complete] Error:", error);
    return {
      success: false,
      error: error.message,
      timestamp: new Date(),
...
export const sendMeetReminders = async () => {
  try {
    ...
    logger.debug(`[Meet Reminders] Found ${upcomingMeets.length} upcoming meets`);
    ...
    logger.error("[Meet Reminders] Error:", error);
    return {
      success: false,
      error: error.message,
      timestamp: new Date(),
...
const sendReminderToEnrolled = async (meet, timeUntil, priority) => {
  try {
    ...
    logger.debug(`[Meet Reminders] Sent ${timeUntil} reminder for: ${meet.title}`);
