...
export const lockCourseAccess = async (userId, courseId, emiPlanId) => {
  ...
  try {
    await sendNotification(userId, "lock", {
      courseId,
      courseName: plan.coursename,
      paymentUrl: "https://schoolemy.com/payment",
    });
...
export const unlockCourseAccess = async (userId, courseId, emiPlanId) => {
  ...
  try {
    await sendNotification(userId, "unlock", {
      courseId,
      courseName: plan.coursename,
      courseUrl: `https://schoolemy.com/course/${courseId}`,
      nextDueDate: plan.emis.find(emi => emi.status === "pending")?.dueDate || new Date(),
    });
  } catch (notificationError) {
    logger.error("Notification error:", notificationError);
    // Don't fail the main operation if notification fails
...
export const processOverdueEmis = async () => {
  ...
  for (const plan of activePlans) {
    try {
      ...
      // Mark as late when DUE DATE passes (not grace period)
      ...
      if (emiStatus.hasOverduePayments && plan.status === "active") {
        ...
        // Send notification
        try {
          await sendNotification(plan.userId, "lock", {
      ...
      // Check if plan should be unlocked (if was locked but now payments are current)
        ...
        // Send notification
        try {
          await sendNotification(plan.userId, "unlock", {
            courseId: plan.courseId,
            courseName: plan.coursename,
            courseUrl: `https://schoolemy.com/course/${plan.courseId}`,
            nextDueDate: plan.emis.find(emi => emi.status === "pending")?.dueDate || new Date(),
          });
        } catch (notificationError) {
          logger.error("Notification error:", notificationError);
      ...
      // CRITICAL BUSINESS FIX: Use 'continue' not 'return' to process all plans
      // Prevents early exit from batch processing when one EMI plan fails
      // Without this, if one plan errors, remaining plans are never processed
      // Customers won't be notified of overdue payments - revenue impact
      logger.error(`Failed to process EMI plan ${plan._id}:`, error.message);
      continue; // Process remaining plans even if this one fails
    }
  }
...
export const sendPaymentReminders = async () => {
  ...
  for (const { emis, ...plan } of upcomingEmis) {
    try {
      await sendNotification(plan.userId, "reminder", {
        courseName: plan.coursename,
...
export const fixEmiStatusForUser = async (userId, courseId) => {
  ...
  try {
    ...
    const today = new Date();
    // Mark as late when DUE DATE passes
