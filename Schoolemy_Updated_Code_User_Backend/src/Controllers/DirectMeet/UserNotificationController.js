...
export const getUserNotifications = async (req, res) => {
  try {
    ...
    logger.debug(` Fetching notifications for user_id: ${user_id}`);
    ...
    logger.debug(` Found ${notifications.length} notifications, ${total} total, ${unreadCount} unread`);
...
export const notifyMeetUsers = async (req, res) => {
  try {
    ...
    logger.error("Error notifying meet users:", error);
    res.status(500).json({
      success: false,
