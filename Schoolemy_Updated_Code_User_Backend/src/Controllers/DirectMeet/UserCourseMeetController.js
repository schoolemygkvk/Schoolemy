...
export const getUserMeets = async (req, res) => {
  try {
    ...
    const query = { course_id: { $in: userCourses } };
    ...
    logger.error("Error fetching user meets:", error);
    res.status(500).json({
      success: false,
...
export const joinMeet = async (req, res) => {
  try {
    ...
    logger.debug(" User enrolledCourses:", user.enrolledCourses);
    logger.debug(" Meet course_id:", meet.course_id._id);
    ...
    logger.debug(" User has access to course:", hasAccess);
    ...
    logger.debug(" Participant found:", participant ? "Yes" : "No");
    if (participant) {
      logger.debug(" Participant payment status:", participant.payment_status);
      logger.debug(" Meet is paid:", meet.is_paid_meet);
    }

    if (participant) {
      logger.debug(" Existing participant found");
      logger.debug(" Payment status:", participant.payment_status);
      logger.debug(" Attendance status:", participant.attendance_status);
      logger.debug(" Meet is_paid_meet:", meet.is_paid_meet);
      logger.debug(" Meet price:", meet.price);
      ...
      if (meet.is_paid_meet && participant.payment_status !== "completed") {
        logger.debug(" Payment required but not completed");
        if (!payment_details) {
          return res.status(403).json({
            success: false,
      ...
      if (participant.attendance_status === "not_joined") {
        logger.debug(" Updating participant from not_joined to joined");
    ...
    logger.error("Error joining meet:", error);
    res.status(500).json({
      success: false,
...
export const completeMeet = async (req, res) => {
  try {
    ...
    logger.error("Error completing meet:", error);
    res.status(500).json({
      success: false,
...
export const getUserMeetHistory = async (req, res) => {
  try {
    ...
    logger.error("Error fetching meet history:", error);
    res.status(500).json({
      success: false,
...
export const validateJoinAccess = async (req, res) => {
  try {
    ...
    logger.error("Error validating join access:", error);
    res.status(500).json({
      canJoin: false,
      reason: "server_error",
