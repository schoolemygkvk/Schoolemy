...
export const getPurchasedCoursesByUser = async (req, res) => {
  try {
    ...
    // SECURITY FIX: Authorization check to prevent IDOR
    // Ensure the userId from URL parameter matches the authenticated user's ID
    if (userId !== req.userId.toString()) {
      return res.status(403).json({ message: "Forbidden: You can only access your own enrolled courses" });
    }
