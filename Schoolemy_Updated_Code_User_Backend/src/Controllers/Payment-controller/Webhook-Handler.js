...
const verifyCashfreeWebhookSignature = (body, timestamp, signature) => {
  try {
    // Cashfree signature verification format: timestamp + raw body (not JSON stringified)
    // body can be either Buffer (raw) or string - convert to string if Buffer
    ...
    // CRITICAL SECURITY FIX: Use timing-safe comparison to prevent timing attacks
    // Prevents attackers from forging signatures by analyzing timing differences
    // Regular === operator can leak signature info through response timing
    try {
...
export const handleCashfreeWebhook = async (req, res) => {
  try {
    ...
    // Verify webhook signature against raw request body (before parsing)
    ...
    // Parse JSON body after signature verification (body is Buffer from express.raw())
    const bodyData = typeof req.body === "string" ? JSON.parse(req.body) : JSON.parse(req.body.toString("utf8"));
...
const handlePaymentSuccess = async (paymentData) => {
  try {
    ...
    if (payment_status !== "SUCCESS") {
      logger.debug("Payment not successful, status:", payment_status);
      return {
        success: false,
    ...
    const result = await withTransaction(async (session) => {
      ...
      if (!payment) {
        logger.debug("Payment record not found for order:", cashfreeOrderId);
    ...
    logger.error("handlePaymentSuccess error:", error);
    return {
      success: false,
...
const handleEmiInstallmentPayment = async (payment, session = null) => {
  try {
    logger.debug("\n" + "=".repeat(60));
    logger.debug(" PROCESSING EMI INSTALLMENT PAYMENT");
    logger.debug(" Payment ID:", payment._id);
    logger.debug(" EMI Plan ID:", payment.emiPlanId);
    ...
    if (emiStatus.paidCount === emiStatus.totalEmis) {
      planUpdates.$set.status = "completed";
      planUpdates.$set.completedAt = new Date();
      hasUpdates = true;
      logger.debug(" EMI plan completed - all installments paid!");
    }

    // Apply plan updates
    if (hasUpdates) {
      // Clean up empty operator objects
      const finalUpdates = {};
      if (Object.keys(planUpdates.$set).length > 0) {
        finalUpdates.$set = planUpdates.$set;
      }
      if (Object.keys(planUpdates.$push).length > 0) {
        finalUpdates.$push = planUpdates.$push;
      }

      await EMIPlan.findByIdAndUpdate(emiPlan._id, finalUpdates, {
      ...
      logger.debug(
        ...
        previousStatus,
        "→",
        planUpdates.$set.status || updatedPlan.status,
      );
    }
    ...
    if (shouldUnlockAccess) {
      logger.debug(" Updating user course access to 'active'...");
      ...
      logger.debug(" User access updated:", {
    ...
    logger.error(" EMI installment payment error:", error);
    throw error; // Re-throw to trigger transaction rollback
  }
};
...
const handlePaymentFailed = async (paymentData) => {
  try {
    ...
    logger.error("handlePaymentFailed error:", error);
    return {
      success: false,
...
const handlePaymentDropped = async (paymentData) => {
  try {
    ...
    logger.error("handlePaymentDropped error:", error);
    return {
      success: false,
