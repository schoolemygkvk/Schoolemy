...
export const getEmiStatus = async (req, res) => {
  try {
    ...
    logger.debug("\n" + "=".repeat(60));
    logger.debug(" EMI STATUS CHECK REQUEST");
    logger.debug(" User ID from auth:", userId);
    logger.debug(" Course ID from params:", courseId);
    ...
    if (!mongoose.Types.ObjectId.isValid(courseId)) {
      logger.error(" Invalid courseId format:", courseId);
    ...
    if (isTutorCourse) {
      ...
      if (tutorPayment) {
        logger.debug(" Tutor course payment found:", {
        ...
        logger.debug(" No payment found for tutor course");
    ...
    logger.debug(" Checking for EMI plan...");
    const emiPlan = await EMIPlan.findOne({
      userId,
      courseId,
    });
    ...
    if (
      ...
      logger.debug(" EMI plan exists but status is:", emiPlan.status);
      // Check if there's a full payment as fallback
      logger.debug(" Checking for full payment (EMI plan is not active)...");
      const fullPayment = await Payment.findOne({
        userId,
        courseId,
        paymentStatus: "completed",
      ...
      if (fullPayment) {
        logger.debug(" Full payment found (EMI plan not active)");
        return res.status(200).json({
          success: true,
          data: {
            paymentType: "full",
      ...
      logger.debug(" Checking for full payment (no EMI plan)...");
      const fullPayment = await Payment.findOne({
        userId,
        courseId,
        paymentStatus: "completed",
      ...
      if (fullPayment) {
        logger.debug(" Full payment found (no EMI plan):", {
    ...
    if (!emiPlan) {
      logger.debug(" EMI plan is null - this should not happen!");
      return res.status(404).json({
        success: false,
    ...
    logger.debug("\n" + "=".repeat(60));
    logger.debug(" EMI Status Calculation for course", courseId);
    logger.debug("=".repeat(60));
    logger.debug(" EMI Status Details:");
    logger.debug(" - Total EMIs:", emiStatus.totalEmis);
    logger.debug(" - Paid EMIs:", emiStatus.paidCount);
    logger.debug(" - Pending EMIs:", emiStatus.pendingCount);
    logger.debug(" - Overdue Count:", emiStatus.overdueCount);
    logger.debug(" - Grace Period Count:", emiStatus.gracePeriodCount);
    logger.debug(" - Upcoming Count:", emiStatus.upcomingCount);
    logger.debug("=".repeat(60));
    logger.debug(" Amount Details:");
    logger.debug(" - Total Amount:", emiStatus.totalAmount);
    logger.debug(" - Total Paid:", emiStatus.totalPaid);
    logger.debug(" - Total Overdue:", emiStatus.totalOverdue);
    logger.debug(" - Total Remaining:", emiStatus.totalRemaining);
    logger.debug(" - Next Due Amount:", emiStatus.nextDueAmount);
    logger.debug("=".repeat(60));
    logger.debug(" Access Control:");
    logger.debug(" - Has Overdue Payments:", emiStatus.hasOverduePayments);
    logger.debug(" - Is Current On Payments:", emiStatus.isCurrentOnPayments);
    logger.debug(" - Has Access To Content:", emiStatus.hasAccessToContent);
    logger.debug(" - Plan Status:", emiPlan.status);
    logger.debug("=".repeat(60) + "\n");
    ...
    logger.debug(" - Should Be Locked:", shouldBeLocked);
    logger.debug(" - Has Due Payments:", emiStatus.hasDuePayments);
    logger.debug(" - Has Overdue Payments:", emiStatus.hasOverduePayments);
    logger.debug(
      " - Reason:",
      shouldBeLocked
        ...
        duePaymentInfo:
          emiStatus.dueEmis.length > 0
            ? {
              ...
              message: ` URGENT: You have ${emiStatus.dueEmis.length} EMI payment(s) due. Your course access is temporarily locked. Please pay immediately to restore access.`,
              alertType: "warning",
            }
            : null,
...
export const payOverdueEmis = async (req, res) => {
  try {
    ...
    logger.debug(" Cashfree order created successfully:", {
      orderId,
      paymentSessionId,
      amount,
    });
    ...
    logger.debug(" Pending payment record created:", payment._id);
    logger.debug(" User will be redirected to Cashfree for payment");
    logger.debug(" Webhook will process payment completion");
    ...
    res.status(200).json({
      ...
      // These will be set after webhook confirms payment
...
export const payMonthlyEmi = async (req, res) => {
  try {
    ...
    // CRITICAL BUSINESS FIX: Create Payment document BEFORE returning to frontend
    // Webhook handler needs this record to find payment and update EMI plan
    // Without this, webhook arrives but can't locate payment record - EMI payments fail silently
    const payment = new Payment({
      userId,
      courseId,
      orderId: orderId,
      ...
      cashfreeOrderId: orderId,
      cashfreePaymentSessionId: paymentSessionId,
      // Store which EMIs will be paid with this amount for webhook handler
      emiInstallments: paymentAllocation.emisToPay.map((emi) => ({
        emiId: emi.emiId,
        month: emi.month,
        monthName: emi.monthName,
    ...
    await payment.save();
    logger.debug(`Payment document created: ${payment._id} for monthly EMI installment(s)`);
...
export const verifyEmiPayment = async (req, res) => {
  try {
    ...
    try {
      ...
      logger.error("Cashfree payment verification error:", cashfreeErr);
