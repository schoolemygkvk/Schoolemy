...
export const updateProfilePicture = async (req, res) => {
  try {
    ...
    console.log(
      "🔧 profile upload Content-Type:",
      req.headers && req.headers["content-type"],
    );
    try {
      console.log(
        "🔧 req.is('multipart/form-data'):",
        typeof req.is === "function" ? req.is("multipart/form-data") : "n/a",
      );
    } catch (e) {
      console.warn("🔧 req.is check failed", e);
    }
    console.log(
      "🔧 req.file present:",
      !!req.file,
    ...
    try {
      ...
      // First, validate the image format by attempting to get metadata
      let sharpInstance;
      try {
        ...
        if (!metadata.format || !supportedFormats.includes(metadata.format)) {
          console.error("⚠️ Unsupported image format:", metadata.format);
          return res.status(400).json({
            success: false,
            message: `Unsupported image format: ${metadata.format || "unknown"}. Please upload JPEG, PNG, WebP, or GIF images.`,
          });
        }
        ...
        console.error("⚠️ Failed to detect image format:", formatError.message);
        return res.status(400).json({
          success: false,
          message:
            "Invalid or corrupted image file. Please upload a valid image (JPEG, PNG, WebP, or GIF).",
      ...
      console.log(
        "📷 Profile picture converted to Base64 successfully (dataUri length)",
        dataUri.length,
      );
    } catch (error) {
    ...
    console.log(
      "🔐 Returning safe profile picture metadata (data omitted) - size:",
      safeProfilePicture.size,
    );
