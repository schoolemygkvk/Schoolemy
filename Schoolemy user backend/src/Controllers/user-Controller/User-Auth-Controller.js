...
export const completeRegistration = async (req, res) => {
  try {
    ...
    // 4. Check again if user already exists by email or mobile
    const query = email ? { email } : { mobile: mobileNumber };
    const existingUser = await User.findOne(query);
    if (existingUser) {
      return res.status(409).json({
    ...
    // 4a. Ensure username and mobile are unique across all users
    if (username) {
...
export const registerForm = async (req, res) => {
  try {
    ...
    // 3a. Ensure email / username / mobile are unique if they are being updated
...
export const verifyForgotPasswordOtp = async (req, res) => {
  try {
    ...
    // Convert mobile to number if provided so it matches how we store it
    const mobileNumber = mobile ? convertMobileToNumber(mobile) : null;
    const query = email ? { email } : { mobile: mobileNumber };
    const user = await User.findOne(query);
...
export const logoutUser = async (req, res) => {
  console.log("Logout API called for user:", req.userId);
  try {
    if (!req.userId) {
      return res.status(401).json({
        success: false,
    ...
    console.log("Found user:", user._id, "current status:", user.status);

    console.log("Before logout - User status:", user.status);
    ...
    try {
      ...
      console.log("Final user status:", user.status);
    } catch (saveError) {
      console.log("Error saving user:", saveError.message);
      throw saveError;
    }

    console.log("After logout - User status:", user.status);
