...
// Indexes
courseMeetSchema.index({ meet_id: 1 }, { unique: true });
  ...
  try {
    ...
    if (this.isNew) {
      ...
      if (enrolledUsers.length > 0) {
        ...
        console.log(`✅ Created ${notifications.length} notifications for meet: ${doc.title}`);
      }
    }
  } catch (error) {
    console.error("Error creating meet notifications:", error);
