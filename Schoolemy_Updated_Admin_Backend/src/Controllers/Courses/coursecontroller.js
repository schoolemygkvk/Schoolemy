...
export const createCourse = async (req, res) => {
  try {
    ...
    while (fixedBody[`chapters[${chapterIndex}].title`]) {
      ...
      while (
        ...
        const lessonData = {
          ...
          audioFile: fileMap[`${base}.audioFile`] || [],
          videoFile: fileMap[`${base}.videoFile`] || [],
          pdfFile: fileMap[`${base}.pdfFile`] || [],
        };
    ...
    const course = new Course({
      ...
      thumbnail: fileMap["thumbnail"]?.[0]?.url || fixedBody.thumbnail || null,
      previewvideo: fileMap["previewvideo"]?.[0]?.url || fixedBody.previewvideo || null,
      contentDuration: {
...
export const createCourseWithS3Urls = async (req, res) => {
  try {
    ...
    // EMI validation (0 is valid for amount; only reject missing/invalid months)
    const emiOn =
      emi?.isAvailable === true || emi?.isAvailable === "true";
    if (emiOn) {
      const months = Number(emi?.emiDurationMonths);
      const totalRaw = emi?.totalAmount;
      const hasTotal =
        totalRaw !== undefined &&
        totalRaw !== null &&
        totalRaw !== "" &&
        !Number.isNaN(Number(totalRaw));
      if (!Number.isFinite(months) || months < 1 || !hasTotal) {
        return res.status(400).json({
          error:
    ...
    const course = new Course({
      ...
      certificates,
      instructor: {
        name: instructor?.name || req.user?.name || "Admin",
        role: instructor?.role || "",
        socialmedia_id: instructor?.socialmedia_id || "",
      },
      description: description || "",
      whatYoullLearn: Array.isArray(whatYoullLearn) ? whatYoullLearn : [],
      emi: {
        isAvailable: emiOn,
        emiDurationMonths: emiOn ? emi?.emiDurationMonths ?? null : null,
        monthlyAmount: emiOn ? emi?.monthlyAmount ?? null : null,
        totalAmount: emiOn ? emi?.totalAmount ?? null : null,
        notes: emi?.notes || "",
      },
    });
...
export const getCourseNames = async (req, res) => {
  try {
    console.log("[COURSES] getCourseNames called - fetching courses from MongoDB");
    res.setHeader('Content-Type', 'application/json; charset=utf-8');

    // Include _id and category for course selection in forms
    console.log("[DB] Executing Course.find() query...");
    const courses = await Course.find({}, "coursename category chapters.title");
    console.log("[DB] Query completed. Found", courses.length, "courses");
    ...
    const formattedCourses = courses.map((course) => {
      ...
      return {
        ...
        chapters: (courseObj.chapters && Array.isArray(courseObj.chapters))
          ? courseObj.chapters.map((chapter) => chapter.title)
          : [],
      };
    });
    ...
    console.error("Failed to get course names:", error?.message);
...
export const updateCourseWithS3Urls = async (req, res) => {
  try {
    ...
    if (fixedBody.previewvideo) existingCourse.previewvideo = fixedBody.previewvideo;
...
export const lambdaUpdateCourse = async (event) => {
  try {
    ...
    if (payload.previewvideo) course.previewvideo = payload.previewvideo;
