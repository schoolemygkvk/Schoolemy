...
export const generateCourseUploadUrls = async (req, res) => {
  try {
    ...
    const TUTOR_BUCKET =
      process.env.AWS_S3_BUCKET_TUTOR || "schoolemy-course-audio-bucket-tutors";
    ...
    const results = await Promise.all(
      files.map(async (fileInfo) => {
        ...
        const contentType =
          (typeof fileType === "string" && fileType.trim()) || "application/octet-stream";

        // Slug: trim, keep words, no duplicate/trailing hyphens (valid S3 key prefix)
        const courseFolder = courseName
          .trim()
          .toLowerCase()
          .replace(/\s+/g, "-")
          .replace(/-+/g, "-")
        ...
        // ContentType must be part of the signature; the browser PUT sends the same header (CourseUpload.js).
        ...
        const s3Url = `https://${BUCKET_NAME}.s3.${AWS_REGION}.amazonaws.com/${encodedKey}`;
