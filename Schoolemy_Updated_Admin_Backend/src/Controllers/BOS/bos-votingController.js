import VotingPoll, { IndividualVote } from "../../Models/BOS/bos-Voting.js";
...
async function pollHasAnyVotes(poll_id, pollDoc) {
  const embedded = Array.isArray(pollDoc?.votes) ? pollDoc.votes.length : 0;
  const totalMarked = Number(pollDoc?.total_votes) || 0;
  if (embedded > 0 || totalMarked > 0) return true;
  const n = await IndividualVote.countDocuments({ poll_id });
  return n > 0;
}
...
export const createVotingPoll = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Error sending voting poll notification emails to recipients:", emailError.message);
      // Don't fail the entire poll creation if email fails
    }

    sendSuccess(res, 201, "Created successfully", newPoll,
    );
  } catch (error) {
    res.status(500).json({
      success: false,
...
export const getAllVotingPolls = async (req, res) => {
  try {
    ...
    const [polls, total] = await Promise.all([
      VotingPoll.find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(parseInt(limit)),
      VotingPoll.countDocuments(query)
    ]);
...
export const getVotingPollById = async (req, res) => {
  try {
    ...
    const hasVotes = await pollHasAnyVotes(id, poll);
    const data = poll.toObject();
    data.can_edit = !hasVotes;
...
export const castVote = async (req, res) => {
  try {
    ...
    // Validate required comments if poll settings require it
    if (poll && poll.settings?.require_comments && !comment?.trim()) {
      return sendError(res, 400, "Comments are required for this poll");
    }
    if (!poll) {
      return res.status(404).json({
        success: false,
    ...
    // Use atomic operations with database-level duplicate protection
    try {
      let result;
      
      if (!poll.allow_multiple_votes) {
        // First, try to insert into IndividualVote collection (database-level protection)
        // This will fail with duplicate key error if user already voted
        try {
          await IndividualVote.create({
            poll_id,
            voter_id: admin._id.toString(),
          ...
          // E11000 duplicate key error - user already voted
        ...
        // Then update the poll's embedded votes array (atomic operation)
        result = await VotingPoll.findOneAndUpdate(
          {
            poll_id,
            "votes.voter_id": { $ne: admin._id.toString() }
          },
          { $push: { votes: vote } },
          { new: true, runValidators: true }
        );
        
        // If poll update failed but IndividualVote succeeded, clean up
        if (!result) {
          await IndividualVote.deleteOne({ poll_id, voter_id: admin._id.toString() });
          return sendError(res, 400, "You have already voted or poll not found");
        }
      } else {
        // Allow multiple votes - no duplicate check needed
        result = await VotingPoll.findOneAndUpdate(
          { poll_id },
          { $push: { votes: vote } },
          { new: true, runValidators: true }
        ...
        // Also record in IndividualVote for tracking (without unique constraint check)
        await IndividualVote.create({
          poll_id,
          voter_id: `${admin._id.toString()}_${Date.now()}`, // Make unique for multiple votes
          voter_name: vote.voter_name,
          voter_role: vote.voter_role,
          option_selected,
          voted_at: vote.voted_at,
      ...
      if (!result) {
        return sendError(res, 400, "Poll not found");
      }

      // Calculate results on the updated poll
      ...
      try {
        ...
        console.error("Error sending vote confirmation email to recipient:", emailError.message);
...
export const updateVotingPoll = async (req, res) => {
  try {
    ...
    if (await pollHasAnyVotes(poll_id, poll)) {
      return res.status(400).json({
        success: false,
...
export const deleteVotingPoll = async (req, res) => {
  try {
    ...
    await IndividualVote.deleteMany({ poll_id });
