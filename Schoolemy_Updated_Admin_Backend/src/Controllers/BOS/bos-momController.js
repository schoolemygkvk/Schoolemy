...
export const createMoM = async (req, res) => {
  try {
    ...
    const meeting = await Meeting.findOne({ meeting_id: String(meeting_id).trim() });
    if (!meeting) {
      return res.status(400).json({
        success: false,
        message: "No meeting found with this Meeting ID. Use the code shown on the meeting (e.g. BOS2026-M008).",
      });
    }
    ...
    sendSuccess(res, 201, "Created successfully", newMoM,
    );
  } catch (error) {
    res.status(500).json({
      success: false,
...
export const getAllMoM = async (req, res) => {
  try {
    ...
    sendSuccess(res, 200, "Success", minutes,
    );
  } catch (error) {
    res.status(500).json({
      success: false,
