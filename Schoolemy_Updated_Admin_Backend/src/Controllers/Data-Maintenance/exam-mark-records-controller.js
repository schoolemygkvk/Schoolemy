...
export const createStudentNewExamRecords = async (req, res) => {
  try {
    ...
    sendSuccess(res, 201, "Created successfully", newRecord,
    );
  } catch (error) {
...
export const getAllStudentExamMarks = async (req, res) => {
  try {
    ...
    // Fetch both collections with pagination applied at DB level
    const [markRecords, markRecordsTotal, examAttempts, examAttemptsTotal] = await Promise.all([
      UserExamAnswer.find(filter).sort(sortObj).skip(skip).limit(limit).lean(),
      UserExamAnswer.countDocuments(filter),
      UserExamAttempt.find(attemptFilter)
        .populate('courseId', 'coursename category')
        .sort({ attemptedAt: sortOrder === 1 ? 1 : -1 })
        .skip(skip)
        .limit(limit)
        .lean(),
      UserExamAttempt.countDocuments(attemptFilter)
    ]);
    ...
    // Combine paginated results from both sources
    const paginatedRecords = [...markRecords, ...convertedAttempts];

    // Calculate total from both collections
    const total = markRecordsTotal + examAttemptsTotal;
