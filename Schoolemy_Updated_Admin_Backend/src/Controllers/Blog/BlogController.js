...
async function uploadBlogImageToS3(buffer, mimeType, originalName) {
  ...
  const rawS3Url = `https://${BLOG_BUCKET}.s3.${AWS_REGION}.amazonaws.com/${key}`;
  return getMediaUrl(rawS3Url); // Use CDN if available
}
...
const createBlog = async (req, res) => {
  try {
    ...
    sendSuccess(res, 201, "Created successfully", blog
    );
  } catch (error) {
...
const getBlogById = async (req, res) => {
  try {
    ...
    sendSuccess(res, 200, "Success", blog
    );
  } catch (error) {
