...
// Separate Vote collection for database-level duplicate protection
const individualVoteSchema = new Schema(
  {
...
// Compound unique index to prevent duplicate votes at database level
// This ensures one voter can only vote once per poll (unless allow_multiple_votes is true)
individualVoteSchema.index({ poll_id: 1, voter_id: 1 }, { unique: true });

export const IndividualVote = model("Bos-Individual-Vote", individualVoteSchema);

// Vote Sub-Schema (kept for backward compatibility in embedded votes array)
