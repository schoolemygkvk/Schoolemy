...
const CreateCourseMeet = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const normalizedCourses = coursesData.map(course => {
        ...
        // If we still don't have a valid ID, log error and skip
        if (!courseId || courseId === '[object Object]') {
          console.error('❌ Invalid course ID for:', course.coursename, 'Raw _id:', course._id);
          return null;
        }
        
        console.log(`✅ Normalized course: ${course.coursename} -> ID: ${courseId} (type: ${typeof courseId})`);
      ...
      if (normalizedCourses.length === 0) {
        ...
        console.error("❌ All courses had invalid IDs");
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (typeof cleanCourseId === 'object' && cleanCourseId !== null) {
        ...
        console.warn("⚠️ course_id was an object, extracted:", cleanCourseId);
      }
      ...
      if (typeof cleanAdminId === 'object' && cleanAdminId !== null) {
        ...
        console.warn("⚠️ adminId was an object, extracted:", cleanAdminId);
      }
      ...
      if (isEditMode) {
        ...
        console.log("📤 Submitting meet data:", submitData);
        console.log("🔑 Final course_id:", submitData.course_id);
        console.log("👤 Final created_by:", submitData.created_by);
        ...
        while (retryCount <= maxRetries) {
          try {
            ...
            if (isDuplicateError && retryCount < maxRetries) {
              retryCount++;
              console.log(`🔄 Duplicate meet ID detected, retrying... (attempt ${retryCount}/${maxRetries})`);
      ...
      console.error(isEditMode ? "❌ Error updating meet:" : "❌ Error creating meet:", error);
  ...
  // Debug: Log courses state before rendering
