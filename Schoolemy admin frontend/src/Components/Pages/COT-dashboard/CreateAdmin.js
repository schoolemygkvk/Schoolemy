...
const CreateAdminForm = () => {
  ...
  // Debug logging - check what role is stored
  React.useEffect(() => {
    console.log("[CreateAdmin] Current User Role from secureStorage:", secureStorage.getItem("role"));
    console.log("[CreateAdmin] Normalized Role:", currentUserRole);
    console.log("[CreateAdmin] Is SuperAdmin:", isSuperAdmin);
    console.log("[CreateAdmin] Available Roles:", roles);
  }, []);
  ...
  // 🔹 Handle Profile Picture (convert to base64 data URL; backend uploads to S3)
  const handleProfilePicture = (e) => {
    ...
    reader.onloadend = () => {
      setFormData((prev) => ({
        ...prev,
        profilePictureBase64: reader.result, // Full data URL: data:image/jpeg;base64,...
      }));
    };
  ...
  const handleSubmit = async (e) => {
    ...
    const currentUserRole = secureStorage.getItem("role") || "";
    if (formData.role === "superadmin" && currentUserRole.toLowerCase() !== "superadmin") {
