...
const TutorCommissionDue = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get("/tutor-commission-due", { headers, params });
  ...
  const handleMarkPaid = async (tutorId, periodId, commissionAmount) => {
    ...
    try {
      const headers = { Authorization: token.startsWith("Bearer") ? token : `Bearer ${token}` };
      await api.post("/tutor-commission/mark-paid", { tutorId, periodId, commissionAmount }, { headers });
      await fetchData();
    } catch (err) {
  ...
  const handleMarkPending = async (tutorId, periodId) => {
    ...
    try {
      const headers = { Authorization: token.startsWith("Bearer") ? token : `Bearer ${token}` };
      await api.post("/tutor-commission/mark-pending", { tutorId, periodId }, { headers });
      await fetchData();
    } catch (err) {
