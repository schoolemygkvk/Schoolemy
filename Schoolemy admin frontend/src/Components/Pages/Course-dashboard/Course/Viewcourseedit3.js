  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        ...
        console.log(
          "📖 Sample lesson names:",
          course.chapters?.[0]?.lessons?.map((l) => l.lessonname),
        );
            ...
            lessons: (chapter.lessons || []).map((lesson) => {
              ...
              if (
                ...
                try {
                  ...
                  console.log(
                    `✅ Parsed audioFile for "${lesson.lessonname}":`,
                    parsedLesson.audioFile,
                  );
                } catch (e) {
                  console.warn(
                    `❌ Failed to parse audioFile for "${lesson.lessonname}":`,
                    e,
                  );
                  parsedLesson.audioFile = [];
                }
              ...
              if (
                ...
                try {
                  ...
                  console.log(
                    `✅ Parsed videoFile for "${lesson.lessonname}":`,
                    parsedLesson.videoFile,
                  );
                } catch (e) {
                  console.warn(
                    `❌ Failed to parse videoFile for "${lesson.lessonname}":`,
                    e,
                  );
                  parsedLesson.videoFile = [];
                }
              ...
              if (
                ...
                try {
                  ...
                  console.log(
                    `✅ Parsed pdfFile for "${lesson.lessonname}":`,
                    parsedLesson.pdfFile,
                  );
                } catch (e) {
                  console.warn(
                    `❌ Failed to parse pdfFile for "${lesson.lessonname}":`,
                    e,
                  );
                  parsedLesson.pdfFile = [];
                }
          ...
          for (const urlData of presignedUrls) {
            ...
            if (file) {
              ...
              if (!uploadResponse.ok) {
                throw new Error(
                  `S3 upload failed for ${file.name}: ${uploadResponse.statusText}`,
                );
              }
