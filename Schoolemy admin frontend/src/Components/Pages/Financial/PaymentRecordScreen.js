...
const PaymentRecordScreen = () => {
  ...
  // Helper function to determine payment type from transaction ID
  const getPaymentTypeFromTxnId = (txnId) => {
    if (!txnId || txnId === 'N/A') return 'unknown';
    const txnLower = txnId.toLowerCase();
    // Transaction IDs starting with "order" are course payments
    if (txnLower.startsWith('order')) return 'course';
    // All other transaction IDs are meet payments
    return 'meet';
  };

  // Group payments by transaction ID with payment type determined by txn ID pattern
  const groupedByTxnId = payments.reduce((acc, payment) => {
  ...
  // Get unique transaction IDs count
  const uniqueTxnIds = Object.keys(groupedByTxnId).filter(id => id !== 'N/A').length;
  
  // Get transaction counts by type based on txn ID pattern
  const courseTxnIds = Object.entries(groupedByTxnId).filter(([id, g]) => g.type === 'course' && id !== 'N/A').length;
  const meetTxnIds = Object.entries(groupedByTxnId).filter(([id, g]) => g.type === 'meet' && id !== 'N/A').length;

  // Filter payments based on search term and type
  const filteredPayments = payments.filter((payment) => {
    const search = searchTerm.toLowerCase();
    
    // Filter by transaction ID if selected
    if (selectedTxnId && payment.transactionId !== selectedTxnId) {
      return false;
    }
    
    // Filter by payment type based on transaction ID pattern
