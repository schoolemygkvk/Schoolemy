...
const SignupModal = ({ open, onClose, onSwitchToLogin, onSignupSuccess }) => { 
    ...
    const resetModal = () => { setActiveStep(0); setRegistrationData({ email: "", mobile: "", password: "" }); setOtpFromBackend(''); onClose(); };
    ...
    const handleVerifyOtp = (data) => { if (data.otp === otpFromBackend) { showToastSuccess("OTP Verified Successfully!"); handleNext(); } else { showToastError("Invalid OTP. Please try again."); } };
    const handleResendOtp = async (userIdentifier) => { try { const response = await authService.resendOtp(userIdentifier); setOtpFromBackend(response.data.otp_for_verification); showToastSuccess(response.data.message || "New OTP sent!"); } catch (error) { showToastError(error.response?.data?.message || "Failed to resend OTP."); } };
  ...
  return (
    <div style={{ fontFamily: theme.font, backgroundColor: "white" }}>
      ...
      <main style={{ minHeight: "80vh" }}>
        <Outlet context={{ onSignUpClick: switchToSignup }} />
      </main>
      <Footer theme={theme} />
      <FloatingAd />
      <LoginModal
