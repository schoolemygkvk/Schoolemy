...
const getProfilePictureUrl = (profilePicture) => {
  ...
  if (profilePicture.data || profilePicture.base64) {
    const rawData = (profilePicture.base64 || profilePicture.data || "").toString().trim();
...
const ProfilePage = () => {
  ...
  const handleUpdateProfileDetails = async (e) => {
    ...
    try {
      const response = await api.put(`/putprofile`, payload);
  ...
  const handleUpdateProfilePicture = async () => {
    ...
    try {
      ...
      const newProfilePictureUrl = getProfilePictureUrl(updatedProfilePicture);

      if (newProfilePictureUrl) {
        setProfilePicturePreview(newProfilePictureUrl);
      } else {
        setProfilePicturePreview(URL.createObjectURL(profilePictureFile));
      }
      ...
      if (cachedUserData) {
        try {
          ...
          parsedData.profilePicture = updatedProfilePicture;
            ...
            {isEditing && profilePictureFile && (
              <div className="upload-actions">
                ...
                <button
                  onClick={() => {
                    setProfilePictureFile(null);
                    setProfilePicturePreview(getProfilePictureUrl(userData.profilePicture));
                    if (fileInputRef.current) fileInputRef.current.value = "";
                  }}
