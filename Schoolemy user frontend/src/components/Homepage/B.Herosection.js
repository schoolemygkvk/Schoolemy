...
const HeroSection = () => {
  ...
  // Use API data only - no default fallbacks
  const eyebrow = hero?.eyebrow ?? "";
  const headline = hero?.headline ?? "";
  const description = hero?.description ?? "";
  const mainImage = hero?.mainImage ?? "";
  const primaryText = hero?.primaryButtonText ?? "";
  const primaryLink = hero?.primaryButtonLink ?? "/course";
  const secondaryText = hero?.secondaryButtonText ?? "";
  const secondaryLink = hero?.secondaryButtonLink ?? "/demo";
  const cardTop = hero?.cardTop ?? null;
  const cardBottom = hero?.cardBottom ?? null;
