...
const API_URL =
  ...
  "https://2sxaoo5hlk.execute-api.ap-south-1.amazonaws.com/dev";
const ADMIN_API_URL =
  ...
  const fetchMeetAndMaterials = async () => {
    try {
      ...
      const materialsResponse = await axios.get(
        `${ADMIN_API_URL}/api/course-meets/materials/s3/${id}?user_id=${userId}`,
        {
      ...
      try {
        const meetResponse = await axios.get(
          `${ADMIN_API_URL}/api/course-meets/meets/${id}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
