...
/**
 * Calculate admin payment to tutors based on completed tutor course payments
 * Query params: dateFrom, dateTo, tutorId (optional), periodId (optional)
 * Returns: Payment details with commission calculations grouped by tutor
 */
...
/**
 * Calculate admin-style payment summary but only for the logged-in tutor
 * Query params: dateFrom, dateTo, periodId (optional)
 * NOTE: Response structure is the same as calculateAdminPaymentToTutors,
 *       but it will only ever contain data for the authenticated tutor.
 */
...
/**
 * Get tutor commission due - all tutors with 15-day commission hierarchy list
 * Query params: periodId (optional - filter by period), status (optional - paid|pending)
 * Returns: Tutors with commission per 15-day period, paid status
 */
...
/**
 * Get payment details for a tutor + period - list of user purchases
 * Query params: tutorId (required), periodId (required)
 * Returns: List of purchases (user, course, amount, commission)
 */
...
/**
 * Mark tutor commission as paid for a 15-day period
 * Body: { tutorId, periodId }
 */
...
/**
 * Mark tutor commission as unpaid/pending (revert)
 * Body: { tutorId, periodId }
 */
