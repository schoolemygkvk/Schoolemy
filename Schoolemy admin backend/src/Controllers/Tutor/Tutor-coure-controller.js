...
export const approveTutorCourse = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to approve course" });
  }
};
...
export const requestChangesForTutorCourse = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to request changes" });
  }
};
...
export const rejectTutorCourse = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to reject course" });
  }
};
...
/**
 * Create tutor course using pre-uploaded S3 files (NO file uploads here)
 * Bypasses API Gateway's 10MB limit - works with live AWS deployment
 *
 * @route POST /createcourses-tutors-with-s3-urls
 * @access Protected (tutor token)
 */
...
/**
 * Update tutor course with pre-uploaded S3 URLs (bypasses API Gateway 10MB limit)
 * @route PUT /course-tutors/update-with-s3-urls/:coursename
 */
export const updateCourseWithS3UrlsForTutor = async (req, res) => {
  try {
    ...
    if (fixedBody.previewVideo && fixedBody.previewVideo[0]?.url) {
      existingCourse.previewvedio = fixedBody.previewVideo[0].url;
    } else if (fixedBody.previewvedio) {
      existingCourse.previewvedio = fixedBody.previewvedio;
    }
