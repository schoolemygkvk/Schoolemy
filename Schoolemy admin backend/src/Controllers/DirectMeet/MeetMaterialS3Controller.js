...
/**
 * Generate presigned URL for uploading meet materials to S3
 * Only for fully attended users in a particular course
 */
  ...
  try {
    ...
    if (!meet_id || !file_name || !file_type) {
      return res.status(400).json({
        success: false,
        message: "meet_id, file_name, and file_type are required"
      });
    }
...
/**
 * Save meet material metadata after successful S3 upload
 * Auto-notify all fully attended users
 */
export const saveMeetMaterial = async (req, res) => {
  try {
    ...
    if (!meet_id || !title || !file_name || !s3_url || !file_type || !material_date) {
      return res.status(400).json({
        success: false,
        message: "meet_id, title, file_name, s3_url, file_type, and material_date are required"
      });
    }
    ...
    if (!uploaded_by) {
      return res.status(400).json({
        success: false,
        message: "uploaded_by is required. Please ensure you are logged in or pass uploaded_by in the request body."
      });
    }
...
/**
 * Get materials for a meet (user access validation)
 */
...
/**
 * Get eligible users for material distribution (full attendance in course)
 */
