  ...
  try {
    ...
    if (!course_id || !title || !description || !meet_date || !meet_time || !duration_minutes) {
      return res.status(400).json({ 
        success: false, 
        message: 'course_id, title, description, meet_date, meet_time, and duration_minutes are required' 
      });
    }
    ...
    if (meet_type === 'online' && !meet_link) {
      return res.status(400).json({
        success: false,
        message: 'meet_link is required for online meets'
      });
    }
    ...
    if (meet_type === 'offline' && !location) {
      return res.status(400).json({
        success: false,
        message: 'location is required for offline meets'
      });
    }
    ...
    try {
      ...
      console.error('⚠️ Meet created but auto-assignment failed:', assignError);
...
export const assignUsersToMeet = async (req, res) => {
  try {
    ...
    if (!meet_id || !user_ids || !Array.isArray(user_ids) || user_ids.length === 0) {
      return res.status(400).json({ 
        success: false, 
        message: 'meet_id and user_ids array are required' 
      });
    }
    ...
    if (users.length === 0) {
      return res.status(400).json({ 
        success: false, 
        message: 'No users found who have purchased this course' 
      });
    }
...
export const markDailyAttendance = async (req, res) => {
  try {
    ...
    if (!meet_id || !user_id || !attendance_date) {
      return res.status(400).json({
        success: false,
        message: 'meet_id, user_id, and attendance_date are required'
      });
    }
