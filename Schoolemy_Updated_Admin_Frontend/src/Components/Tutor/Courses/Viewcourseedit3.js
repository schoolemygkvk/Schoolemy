...
const EditCourse = () => {
    ...
    const fetchCourse = async () => {
      try {
        ...
        setFormData({
          ...
          contentduration: course.contentDuration || course.contentduration || { hours: 0, minutes: 0 },
          whatYoullLearn: course.whatYoullLearn || [],
          chapters: parseMediaFiles(course.chapters || []),
          thumbnail: course.thumbnail || null,
          previewVideo: course.previewvideo || course.previewVideo || null,
          emi: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setUploadHadFiles(fileRequests.length > 0);

      // Upload files to S3 if any
      if (fileRequests.length > 0) {
        // Validate course name - use formData or fallback to URL parameter
        ...
        for (let i = 0; i < presignedUrls.length; i++) {
          ...
          if (!putRes.ok) {
            ...
            throw new Error(
              `Failed to upload ${file.name} (HTTP ${putRes.status}): ${detail || putRes.statusText}`,
            );
          }
      ...
      if (formData.thumbnail instanceof File) {
        const url = s3UploadedFiles.thumbnail?.[0]?.url;
      ...
      if (formData.previewVideo instanceof File) {
        const url = s3UploadedFiles.previewvideo?.[0]?.url;
        if (url) updateData.previewvideo = url;
      } else if (typeof formData.previewVideo === "string" && formData.previewVideo) {
        updateData.previewvideo = formData.previewVideo;
      }
        ...
        return (
          <Paper sx={styles.section}>
            ...
            <Grid container spacing={3}>
              ...
              {/* Thumbnail & preview video — same as admin course edit */}
              <Grid item xs={12} md={6}>
                <Box>
                  <Typography
                    variant="subtitle2"
                  ...
                  >
                    ...
                    <input
                      ...
                      name="thumbnailFile"
                      onChange={(e) => {
                        if (e.target.files?.[0]) handleChange(e);
                      }}
                    />
                  </Button>
                  {formData.thumbnail && (
              ...
              <Grid item xs={12} md={6}>
                <Box>
                  ...
                  >
                    Upload or replace preview video
                    <input
                      type="file"
                      hidden
                      accept="video/*"
          ...
          >
            ...
            <Box sx={styles.navigation}>
              ...
              <Button
                type="submit"
                sx={styles.buttonPrimary}
                disabled={isLoading || uploadStarted}
                startIcon={
  ...
  return (
    <Box
      ...
      sx={{
        ...styles.container,
        ...(uploadStarted ? { paddingBottom: "200px" } : {}),
      }}
    >
      <Box
        sx={{
