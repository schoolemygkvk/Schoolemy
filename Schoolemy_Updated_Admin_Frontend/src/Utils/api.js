...
if (!API_BASE_URL) {
  ...
  throw new Error("REACT_APP_API_URL is required. Please check your .env configuration.");
}
...
api.interceptors.request.use(
  (config) => {
    ...
    // Add CSRF token for mutating requests (cookies sent automatically via withCredentials)
...
const _clearAuthAndRedirect = () => {
  ...
  if (window.location.pathname !== "/") {
    // Dispatch custom event for soft navigation instead of hard reload
    console.log("[API] Dispatching auth:logout event for soft navigation");
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Handle 401 Unauthorized - Try refresh on TOKEN_EXPIRED or TOKEN_EXPIRING_SOON, go to login otherwise
    if (error.response?.status === 401) {
      const code = error.response?.data?.code;
      console.error(`[API] 401 Unauthorized (${code}) - ${fullUrl}`);

      // TOKEN_EXPIRED or TOKEN_EXPIRING_SOON: try to refresh before giving up
      if (code === 'TOKEN_EXPIRED' || code === 'TOKEN_EXPIRING_SOON') {
        // Prevent infinite retry loop — if this request was already retried, don't retry again
        if (error.config._retry) {
          console.error(`[API] Token refresh retry limit exceeded for ${fullUrl}`);
          _clearAuthAndRedirect();
          return Promise.reject(error);
        }

        // Mark this request as having been retried once
        error.config._retry = true;

        try {
          // Attempt refresh with stored refresh token in body
          const refreshToken = secureStorage.getItem("refreshToken");
          const refreshRes = await api.post('/refresh-token', { refreshToken }, { noAuth: true });

          // Store the new access token and refresh token
          if (refreshRes.data?.accessToken) {
            secureStorage.setItem("token", refreshRes.data.accessToken);
            // Update the failed request with new token
            error.config.headers["Authorization"] = `Bearer ${refreshRes.data.accessToken}`;
          }
          if (refreshRes.data?.refreshToken) {
            secureStorage.setItem("refreshToken", refreshRes.data.refreshToken);
          }
          ...
          console.error(`[API] Token refresh failed`, refreshError.response?.data);
          _clearAuthAndRedirect();
        }
      } else {
        // NO_REFRESH_TOKEN / INVALID_TOKEN / other 401 — go to login immediately
        _clearAuthAndRedirect();
      }
    }
